function render(ctx: CanvasRenderingContext2D) {
	...
	function renderFenster(fn: Fenster) {
		...
		function backgrounds() {
			...
			if (backgroundsColor.content) {
				...
				ctx.fillRect(
					x + sizes.border,
					y + sizes.border + sizes.padding + sizes.title,
					sx - sizes.border * 2,
					sy - sizes.border * 2 - sizes.padding - sizes.title,
				);
			}
		}
		function buttons() {
			...
			if (buttonsConfig.close.enable) {
				if (backgroundsColor.button.close) {
					...
					ctx.fillRect(
						xx - sizes.buttons.close,
						y + sizes.border + sizes.buttons.border,
						sizes.buttons.close,
						sizes.title - sizes.buttons.border * 2,
					);
				}
				const img = getAsset(buttonsConfig.close.name)?.src as HTMLImageElement;
				if (img)
					ctx.drawImage(
						img,
						xx - sizes.buttons.close,
			...
			if (buttonsConfig.max.enable) {
				if (backgroundsColor.button.max) {
					...
					ctx.fillRect(
						xx - sizes.buttons.max,
						y + sizes.border + sizes.buttons.border,
						sizes.buttons.max,
						sizes.title - sizes.buttons.border * 2,
					);
				}
				const img = getAsset(buttonsConfig.max.name)?.src as HTMLImageElement;
				if (img)
					ctx.drawImage(
						img,
						xx - sizes.buttons.max,
			...
			if (buttonsConfig.min.enable) {
				if (backgroundsColor.button.min) {
					...
					ctx.fillRect(
						xx - sizes.buttons.min,
						y + sizes.border + sizes.buttons.border,
						sizes.buttons.close,
						sizes.title - sizes.buttons.border * 2,
					);
				}
				const img = getAsset(buttonsConfig.min.name)?.src as HTMLImageElement;
				if (img)
					ctx.drawImage(
						img,
						xx - sizes.buttons.min,
	...
	for (const fn of fensters) {
		fn.runRender(0);
		fn.runRender(1);
		if (fensters.indexOf(fn) == 0) fn.runRender(2);
		fn.runRender(3);
		renderFenster(fn);
	}
}

function tryFont(name: string, url?: string, defaultFont = "Roboto") {
	...
	if (asset) {
		...
		if (url) new Asset(url, "font", name);
		return defaultFont;
	}
}
